...
Optional ByVal sCanALL As Boolean = True, Optional tipe As Long = 1)
...
lewaT:
    ...
    Case 2
    scanNOWRTP szFullPath, ojanIsFolder
    End Select
    End If
...
erH:
End Sub
Private Sub scanNOWRTP(path As String, bAdalahFolder As Boolean)
On Error GoTo aK
Dim pjG As Long
If bAdalahFolder = False Then
pjG = scE.lenFileEX(path)
If pjG <= 2 Then Exit Sub
If pjG > 1750000 Then Exit Sub
    CekVirus path, pjG, , , 2
End If
aK:
pjG = vbNull
...
Optional VBZ As Boolean = False, Optional tipe As Long = 1) As Boolean
...
Select Case tipe
    ...
    'RtpScan FileNOW
    Case 3
    Do: DoEvents: Sleep 10: Loop While loADingF = True
    FgrdEnd = False
    Fguard.showREPORT viruzE, FileNOW, tipeVirus, pId
    Do: DoEvents: Sleep 10: Loop While FgrdEnd = False
End Select
End Function
Sub RtpScan(FileNOW As String)
Dim lis As cListItem
FrtpUser.Show
Set lis = FrtpUser.ucListView1.ListItems.Add(, viruzE)
...
Dim lis As cListItem
Set lis = FrmScanner.ucListView1(0).ListItems.Add(, viruzE)
lis.SubItem(2).Text = FileNOW
lis.SubItem(3).Text = tipeVirus
Set lis = Nothing
jmlVIR = jmlVIR + 1
FrmScanner.Label1(1).Caption = "Virus (" & jmlVIR & ")"
End Sub
Public Sub scanProC()
...
If (NAT = 2 Or NAT = 34 Or NAT = 3 Or NAT = 6 Or NAT = 22 Or NAT = 18 Or NAT = 50 Or NAT = 19 Or NAT = 35) Then
    FrmScanner.ucListView1(2).ListItems.Add(, ObjName).SubItem(2).Text = sFile
    hiddenFile = hiddenFile + 1
    FrmScanner.Label1(4).Caption = "Hidden (" & hiddenFile & ")"
End If
