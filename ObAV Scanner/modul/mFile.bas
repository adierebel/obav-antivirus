...
Set fso = Nothing
End Function
Public Sub restoreQuar(Lv As ucListView)
On Error Resume Next
Dim LvCount As Long, i As Long
Dim oriPath As String
LvCount = Lv.ListItems.Count
For i = 1 To LvCount
If Lv.ListItems.Item(LvCount).Checked = True Then
oriPath = Lv.ListItems.Item(LvCount).SubItem(3).Text
SetFileAttributesW (StrPtr(oriPath)), vbNormal
    If scE.FileAdaX(oriPath) = True Then
    If MsgBox("Replace file ?", vbQuestion + vbYesNo) = vbYes Then
    DeleteFileW StrPtr(oriPath)
    Else
    GoTo exitZ
    End If
    End If
    encripFile Lv.ListItems.Item(LvCount).SubItem(2).Text, oriPath
End If
exitZ:
LvCount = LvCount - 1
DoEvents
...
exitS:
End Sub
Public Sub exsekusiReg(Lv As ucListView)
Dim i As Long, j As Long
...
For i = 1 To j
If Lv.ListItems.Item(j).Checked = True Then
    If InStr(Lv.ListItems.Item(j).SubItem(3).Text, "startup") > 0 Then
    tmp = Lv.ListItems.Item(j).SubItem(2).Text
    pathReg = Right$(tmp, Len(tmp) - 1)
      If Left$(tmp, 1) = "a" Then
      DelSetting HKEY_CURRENT_USER, pathReg, Lv.ListItems.Item(j).Text
      ElseIf Left$(tmp, 1) = "b" Then
      DelSetting HKEY_LOCAL_MACHINE, pathReg, Lv.ListItems.Item(j).Text
      End If
      Lv.ListItems.Remove j
    End If
End If
