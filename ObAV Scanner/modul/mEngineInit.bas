...
Private Sub cekMesin()
    Load Fscann
    Fscann.Show
    If FindWindow("#32770", "obav_22091993") < 2 Then
      If MsgBox("ObAV Guard Not Active, Activate now ?", vbYesNo + vbQuestion) = vbYes Then _
...
'MsgBox getcoMan(4)
'Exit Sub
Dim pShellMenu As String
Call initializeALL
If getcoMan(2) = "-Scan" Then
    pShellMenu = getcoMan
    If Len(pShellMenu) > 1 Then
    If ValidFile(pShellMenu) = False Then
    ScanEXT pShellMenu, False
    Else
    ScanEXT pShellMenu, True
    End If
    End If
Else
