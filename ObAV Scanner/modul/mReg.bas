...
Dim CNT As Long, buf As String, Buf2 As String, retdata As Long, typ As Long
    ...
    While RegEnumValueUn(Result, CNT, StrPtr(buf), RET, 0, typ, ByVal StrPtr(Buf2), retdata) <> ERROR_NO_MORE_ITEMS
        If typ = REG_DWORD Then
            KeyName = VBA.Left(buf, RET)
            If Trim$(Buf2) <> "" Then KeyPath = FixStartP(VBA.Left$(Asc(Buf2), retdata - 1))
        Else
            KeyName = VBA.Left(buf, RET)
            If Trim$(Buf2) <> "" Then KeyPath = FixStartP(VBA.Left$(Buf2, retdata - 1))
        End If
        
        Set lsv = Lv.ListItems.Add()
        lsv.Text = KeyName
        lsv.SubItem(2).Text = KeyPath
        If Start = a Then
            lsv.SubItem(3).Text = "HKEY_CURRENT_USER\SOFTWARE\Microsoft\Windows\CurrentVersion\Run"
        ElseIf Start = b Then
            lsv.SubItem(3).Text = "HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows\CurrentVersion\Run"
        End If
    ...
    RegCloseKey Result
End Sub
Public Sub GetSTARTUP(Lv As ucListView)
Lv.ListItems.Clear
getValSta a, Lv
getValSta b, Lv
End Sub
...
While RegEnumValueUn(Result, CNT, StrPtr(buf), RET, 0, typ, ByVal StrPtr(Buf2), retdata) <> ERROR_NO_MORE_ITEMS
    ...
    If CekVirus(KeyPath, pjG) = True Then
        Set lis = Fscann.ucListView1(1).ListItems.Add(, KeyName)
        If Start = a Then
        ...
        Fscann.tabson.GantiJudul 4, "Registry @ " & registriBad
    End If
lWT:
    CNT = CNT + 1
    buf = Space$(BUFFER_SIZE)
