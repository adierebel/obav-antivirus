...
Begin VB.UserControl DirTree 
   ...
   Begin VB.PictureBox picBuffer 
      ...
      Width           =   240
   End
   Begin ObAVScanner.ucTreeView tree1 
      Height          =   3255
      Left            =   0
      TabIndex        =   0
      Top             =   0
...
End Sub
Public Sub LoadTreeDir(Optional cekSis As Boolean, Optional cekKomp As Boolean, Optional cekSpes As Boolean, Optional cekWindows As Boolean, Optional cekProgFiles As Boolean)
    Dim DriveNum      As Integer, node1 As Long, DriveType As Long, ikonku As Integer, nodeKomp As Long, nodeSis As Long, NodePathLain  As Long, NodePathWindows As Long, NodePathProgFiles As Long
    DriveNum = 64: tree1.Clear: SelesaiLoad = False
    nodeKomp = AddFolder(tree1, "My Computer", False, , "::komputer", 5, 5)
    
       ' Tambahkan Path Lain
    NodePathLain = AddFolder(tree1, "Special Directory", False, , "::plain", 0, 0)
    NodePathWindows = AddFolder(tree1, "Windows", True, NodePathLain, "::pwin", 0, 0)
    NodePathProgFiles = AddFolder(tree1, "Program Files", True, NodePathLain, "::ppro", 0, 0)
    AddFolder tree1, "My Documents", True, NodePathLain, "::pdoc", 10, 10
    
    
    'list untuk sistem
    nodeSis = AddFolder(tree1, "System Area", False, , "::sistem", 6, 6)
    AddFolder tree1, "Memory Process", True, nodeSis, "::spro", 7, 7
    AddFolder tree1, "Registry", True, nodeSis, "::sreg", 8, 8
...
lanjutkan:
    ...
    tree1.Expand NodePathLain
    
    If cekSis = True Then
    tree1.NodeChecked(nodeSis) = True
    tree1.CheckChildren nodeSis, True
    tree1_NodeCheck nodeSis
    End If
     If cekProgFiles = True Then
    tree1.NodeChecked(NodePathProgFiles) = True
    tree1.CheckChildren NodePathProgFiles, True
    tree1_NodeCheck NodePathProgFiles
    End If
    If cekWindows = True Then
    tree1.NodeChecked(NodePathWindows) = True
    tree1.CheckChildren NodePathWindows, True
    tree1_NodeCheck NodePathWindows
    End If
     If cekSpes = True Then
    tree1.NodeChecked(NodePathLain) = True
    tree1.CheckChildren NodePathLain, True
    tree1_NodeCheck NodePathLain
    End If
    If cekKomp = True Then
    tree1.NodeChecked(nodeKomp) = True
    tree1.CheckChildren nodeKomp, True
    tree1_NodeCheck nodeKomp
    End If
End Sub
